package framework.components.Login.Application.presentationModel
{
	...
	public class MainScreenModel extends EventDispatcher { //20160825 EPMModel {  
		 
		var loginCmd:LoginCommand; //20160825
		...
		/*20160825
		[Bindable]	
		public var model:IMainAppGlobalModel; //  
		*/
...
*/
		...
		public  function sendLoginRequestMSG(typeParam:int, loginDTOParam:LoginDTO):void {
			...
			try{
				...
				//a) Cmd conf:
				//20160825 var loginCmd:LoginCommand = new LoginCommand; //LO MANDO INSTANCIADO AL PPIO EN EL CONSTRUCTOR xa 
																		//poder poner listeners una unica vez...
		...
		public function MainScreenModel(){
			...
			//20160825 Faltarian los listeners mepa (?) 
			loginCmd= new LoginCommand;
			loginCmd.addEventListener(Messages.LOGIN_DENIED_RESPONSE_MESSAGE, loginResultEvtHandlerWrapper);
			loginCmd.addEventListener(Messages.LOGIN_GRANTED_RESPONSE_MESSAGE, loginResultEvtHandlerWrapper);
			loginCmd.addEventListener(Messages.LOGIN_FAIL_RESPONSE_MESSAGE, loginResultEvtHandlerWrapper);
			
		}
		
		//20160825
		public function loginResultEvtHandlerWrapper(evt:EFlexEvent):void {  //(evt:FlexEvent):void { //(evt:Event):void { 
			//trace("llegó"); 
			try {
				var dummyEvent:EFlexEvent =  evt as EFlexEvent; //new FlexEvent(evt.type); //evt as EFlexEvent;
				//20160825 
				dummyEvent.data=evt.data;
				//20160825 dummyEvent[data]=evt.type;
				//dummyEvent["data"]=evt.type;
