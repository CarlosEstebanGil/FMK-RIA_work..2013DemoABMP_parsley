...
*/ 
		...
		//20160825 override public function action(data:Object):void { //(dataXMLCollection:XMLListCollection):void {
		//20160825 public function action(data:Object):void { //(dataXMLCollection:XMLListCollection):void {
		override public function action(data:Object):void { //(dataXMLCollection:XMLListCollection):void {
			...
			var response_message:EFlexEvent; //20160825 EFlexEvent; //20140520 var response_message:IMessage
			try {
				//20140702 r = data.result.qry.item.userValid as int; 
				//20160826 EL API DE ESERVICE ES VIEJO Y ME DEVUELVE XMLLISTCOLLECTION (YO LUEGO LO HABIA CAMBIADO A QUE DEVUELVA EL RESULT OBJECT SIN PRE-PROCESAR ent se accedia asi
				//r= data.result.qry.item.attribute("userValid").toString();
				//ahora como me manejo con el api viejo de xmllistcollection pre-procesado no me qda otra q accesar al nodo recorriendo el xmllistcollection 
				
				for each(var item in data){
					r= int(item.attribute("userValid"));
				}
...
//		
					...
					response_message = new EFlexEvent(Messages.LOGIN_GRANTED_RESPONSE_MESSAGE); //20160825 new EFlexEvent(Messages.LOGIN_GRANTED_RESPONSE_MESSAGE); //xq necesito la data del user.
					...
					*/
					...
					//20141018
					//20160825 dataOut.language = loginDTO.language;
...
//					sendMessage(response_message); //-> El modulo/comp usuario debe estar escuchando x esta kte msge id 79! //(deleteRequestMessage); 
				//	dispatch(response_message);
					...
					response_message = new EFlexEvent(Messages.LOGIN_DENIED_RESPONSE_MESSAGE); //20160825 EFlexEvent(Messages.LOGIN_DENIED_RESPONSE_MESSAGE); //("LOGIN_DENIED"); //new EMessage(Messages.LOGIN_DENIED_RESPONSE_MESSAGE); 
				//	dispatch(response_message);
				...
				//20160825 Logger.logError(LOG, error); 
				//throw new Error(error.message);}
				//response_message = new EMessage(Messages.LOGIN_FAIL_RESPONSE_MESSAGE); 
				response_message = new EFlexEvent(Messages.LOGIN_FAIL_RESPONSE_MESSAGE); //20160825 EFlexEvent(Messages.LOGIN_FAIL_RESPONSE_MESSAGE); //("LOGIN_FAILED");
		...
		//209160825 override public function fault(data:Object):void { //(dataXMLCollection:XMLListCollection):void {
		//209160825 public function fault(data:Object):void { //(dataXMLCollection:XMLListCollection):void {
...
//			sendMessage(response_message); //
			...
			//20160825 dispatch(response_message);
			dispatchEvent(response_message);
		} 
